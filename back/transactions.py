...
def sell_stock():
    ...
    # ✅ Fetch stock price
    stock_data = execute_query("SELECT price FROM Stock WHERE ticker = %s", (ticker,), fetch=True, fetchone=True)
    if not stock_data:
        flash("Stock not found")
        return redirect(url_for("portfolio.view_portfolio"))
    ...
    portfolio_entry = execute_query(
        "SELECT quantity FROM Portfolio WHERE user_id = %s AND stock_ticker = %s", 
        (current_user.user_id, ticker), fetch=True, fetchone=True
    )
